...
__export(external_message_converter_exports, {
  convertExternalMessages: () => convertExternalMessages,
    ...
    if (output.role === "tool") {
      ...
      if (toolCallIdx !== -1) {
        ...
        assistantMessage.content[toolCallIdx] = {
          ...toolCall,
          ...{
            [import_getExternalStoreMessage.symbolInnerMessage]: [
              ...toolCall[import_getExternalStoreMessage.symbolInnerMessage] ?? [],
              output
            ]
          },
      ...
      const content = (typeof output.content === "string" ? [{ type: "text", text: output.content }] : output.content).map((c) => ({
        ...c,
        ...{ [import_getExternalStoreMessage.symbolInnerMessage]: [output] }
      }));
      switch (role) {
        case "system":
        case "user":
...
var useExternalMessageConverter = ({
  ...
  return (0, import_react.useMemo)(() => {
    ...
    const threadMessages = state.converterCache.convertMessages(
      ...
      (cache, message, idx) => {
        ...
        if (cache && (cache.role !== "assistant" || !(0, import_auto_status.isAutoStatus)(cache.status) || cache.status === autoStatus)) {
          const inputs = (0, import_getExternalStoreMessage.getExternalStoreMessages)(cache);
    ...
    threadMessages[import_getExternalStoreMessage.symbolInnerMessage] = messages;
